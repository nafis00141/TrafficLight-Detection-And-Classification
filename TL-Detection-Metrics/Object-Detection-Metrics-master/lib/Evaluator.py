...
class Evaluator:
    ...
    def PlotPrecisionRecallCurve(self, classId, boundingBoxes, IOUThreshold=0.5, showAP=False, showInterpolatedPrecision=False, savePath=None, showGraphic=True):
        ...
        """
        IOUThreshold2 = 0.3
        results1 = self.GetPascalVOCMetrics(boundingBoxes, IOUThreshold)
        results2 = self.GetPascalVOCMetrics(boundingBoxes, IOUThreshold2)
        ...
        average_precision1 = result1['AP']
        mpre1 = result1['interpolated precision']
        mrec1 = result1['interpolated recall']
        npos1 = result1['total positives']
        total_tp1 = result1['total TP']
        total_fp1 = result1['total FP']
        
        precision2 = result2['precision']
        recall2 = result2['recall']
        average_precision2 = result2['AP']
        mpre2 = result2['interpolated precision']
        mrec2 = result2['interpolated recall']
        npos2 = result2['total positives']
        total_tp2 = result2['total TP']
        total_fp2 = result2['total FP']

        if showInterpolatedPrecision:
            plt.plot(mrec, mpre, '--r' , label='Interpolated precision')
        plt.plot(recall1, precision1, label='threshold .5',)
        plt.plot(recall2, precision2, label='threshold .3')
        plt.xlabel('recall')
        plt.ylabel('precision')
        if showAP:
            ap_str1 = "{0:.2f}%".format(average_precision1*100)
            ap_str2 = "{0:.2f}%".format(average_precision2*100)
            plt.title('Precision x Recall curve \nClass: %s \nAP: %s for IOU 0.5 \nAP: %s for IOU 0.3' % (str(classId), ap_str1,ap_str2))
        ...
        ret ={}
        ret['class'] = classId
        ret['precision'] = precision1
        ret['recall'] = recall1
        ret['AP'] = average_precision1
        ret['interpolated precision'] = mpre1
        ret['interpolated recall'] = mrec1
        ret['total positives'] = npos1
        ret['total TP'] = total_tp1
        ret['total FP'] = total_fp1
        return ret
